...
class UsersController extends Controller
{
    ...
    {
        $roles = array_keys($request->input('roles', []));
        // jika user admin kelurahan
        $rules = [];
        if (in_array(3, $roles)) {
            $rules = [
        ...
        $user = $userModel::withTrashed()->updateOrCreate(['username' => $input['username']], $input);
        $user->restore();
    ...
    public function edit($id)
    {
        ...
        return view('boilerplate::users.edit', [
            ...
            'roles' => Auth::user()->hasRole('admin') ? $role::all() : $role::whereNotIn('name', ['admin'])->get(),
            'kecamatan' => Kecamatan::get(),
            'keldes'    => KelDesa::find($user->id_kelurahan),
        ]);
    }
    ...
    public function firstLoginPost(Request $request)
    {
        ...
        Auth::attempt(['username' => $user->username, 'password' => $request->input('password'), 'active' => 1]);
    ...
    public function avatarUpload(Request $request)
    {
        ...
        try {
            if ($avatar && $avatar->isValid()) {
                ...
                $fileName = md5($user->id.$user->username).'_tmp.'.$extension;
